...
class BACustomerMembership: UIViewController {
    ...
    func getDashoard() {
        ...
        let gymId =    UserDefaults.standard.string(forKey: "GymId") ?? ""
             
        let params =  "strXml=<NeoFitnes><Credential><UserName>\(userName)</UserName><Password>\(password)</Password></Credential><CustomerMembershipExpiry><MembershipExpiry><BranchId>\(branchId)</BranchId><GymId>\(gymId)</GymId><StartDate>\(startDate)</StartDate><EndDate>\(endDate)</EndDate><PackageCategoryId>0</PackageCategoryId></MembershipExpiry></CustomerMembershipExpiry></NeoFitnes>"
        ...
        let data = params.data(using: .utf8, allowLossyConversion: true)
           ...
           // let todosEndpoint: String = "http://mobile.rightsoft.in/NeoFitnessService.asmx/GetAllBranchWiseGroupClasses"
            let todosEndpoint: String = "http://mobile.rightsoft.in/NeoFitnessService.asmx/GetCustomerMembershipExpiryReport"
...
extension BACustomerMembership: XMLParserDelegate {
    ...
    func parser(_ parser: XMLParser, didEndElement elementName: String, namespaceURI: String?, qualifiedName qName: String?) {
        if elementName == "CustomerMembershipExpiry" {
            ...
            let membership = CustomerMembership(CustomerName: CustomerName, ContactNo: ContactNo, ReferredBy: ReferredBy, PackageName: PackageName, BranchId: BranchId, MembershipStartDate: MembershipStartDate, MembershipExpiryDate: MembershipExpiryDate, LastAttendanceDate: LastAttendanceDate)
            self.customerMembership.append(membership)
    ...
    func parserDidEndDocument(_ parser: XMLParser) {
        ...
        DispatchQueue.main.async {
            ...
            if self.customerMembership.count == 0 {
                self.NoDataPopUp()
            }
...
extension BACustomerMembership: UITableViewDelegate, UITableViewDataSource {
    
    func numberOfSections(in tableView: UITableView) -> Int {
        self.countLabel.text = "Count: \(customerMembership.count)"
        return customerMembership.count
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.customerNameLabel.text = "Customer Name: \t \t \t \(customerMembership[indexPath.section].CustomerName)"
        cell.contactNumberLabel.text = "ContactNo: \t \t \t \t \(customerMembership[indexPath.section].ContactNo)"
        cell.categoryTypeLabel.text = "Category Type: \t \t  \t \(customerMembership[indexPath.section].ReferredBy)"
        cell.packageLabel.text = "Package: \t \t \t \(customerMembership[indexPath.section].PackageName)"
        cell.membershipStartDatelabel.text = "Start Date: \t \t \t \t \(self.dateString(dateString: customerMembership[indexPath.section].MembershipStartDate))"
        cell.membershipExpiryLabel.text = "End Date: \t \t \t \t \(self.dateString(dateString:customerMembership[indexPath.section].MembershipExpiryDate))"
        cell.attendenceLabel.text = "Last Attendence: \t \t \t \(self.dateString(dateString:customerMembership[indexPath.section].LastAttendanceDate))"
